...
const paceWidth = computed(() => Math.round(pageWrapperScrollTop.value / pageWrapperScrollHeight.value * 100));

const onScroll = useThrottleFn(() => {
  if (window.innerWidth < 1210) {
    pageWrapperScrollTop.value = Math.round(pageWrapper.value?.scrollTop ?? 0);
  }
}, 300);
